...
function hasStyle(node, property, value) {
  ...
  const textDecoration = o.stylesheet.rules[0].declarations.find(
    (d) => d.property.toLowerCase() === property,
  )
...
function getConverter() {
  ...
  const converter = new turndown({
    headingStyle: 'atx',
    hr: '---',
    bulletListMarker: Zotero.Prefs.get(
      'extensions.mdnotes.html2md.default.bullet',
      true,
    ),
    strongDelimiter: Zotero.Prefs.get(
      'extensions.mdnotes.html2md.default.strong',
      true,
    ),
  ...
  converter.addRule('strikethrough', {
    ...
    filter: (node) => {
      return (
        hasStyle(node, 'text-decoration', 'line-through') ||
        node.nodeName === 'S' ||
        node.nodeName === 'DEL' ||
    ...
    replacement: (content) => {
      const delimiter = Zotero.Prefs.get(
        'extensions.mdnotes.html2md.default.strikethrough',
        true,
      )
  ...
  converter.addRule('underline', {
    ...
    replacement: (content) => {
      const open = Zotero.Prefs.get(
        `extensions.mdnotes.html2md.rules.underline.open`,
        true,
      )
      const close = Zotero.Prefs.get(
        `extensions.mdnotes.html2md.rules.underline.close`,
        true,
      )
...
const MarkdownUtils = new (class {// tslint:disable-line:variable-name
  /**
   ...
   */
  addBlockIds(md: string, citekey) {
    const lines = md.split('\n')
    let new_md = ''

    // Loop through the file
    for (const line of lines) {
      const blockId = this.generateBlockId(line, citekey)
  ...
  generateBlockId(str: string, citekey: string) {
    const hash_length = 7
    const ck = citekey ? citekey + '-' : ''
    const hash = shajs('sha256').update(str).digest('hex').substr(0, hash_length)
    return `${ck}${hash}`
  }
})()
